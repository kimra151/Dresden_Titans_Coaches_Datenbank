...
export default function SubmitContentModal({ isOpen, onClose }: SubmitModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Optional: Du kannst hier auch die anderen Metadaten an die Server Action übergeben
      formData.append('title', title);
      formData.append('category', category);
      formData.append('level', level);
      formData.append('description', description);

      // Aufruf der Cloudflare R2 Server Action
      const result = await getPresignedUrl(formData);

      if (result.success) {
        setUploadSuccess(true);
        // Timeout, um dem User den Erfolg zu zeigen, bevor das Modal schließt
        setTimeout(() => {
          onClose();
          // Hier States zurücksetzen für den nächsten Upload
          setFile(null);
          setStep(1);
